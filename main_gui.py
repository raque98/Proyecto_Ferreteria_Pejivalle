...
from dao.dao_devoluciones import DAODevoluciones
...
class FerreteriaGUI:
    ...
    def conectar(self):
        try:
            ...
            self.dao_devoluciones = DAODevoluciones(self.connection)
    ...
    def menu_productos(self):
        ...
        ttk.Button(frame, text="Registrar categoría", command=self.registrar_categoria_gui, width=25).pack(pady=3)
        ttk.Button(frame, text="Actualizar categoría", command=self.actualizar_categoria_gui, width=25).pack(pady=3)
        ttk.Button(frame, text="Eliminar categoría", command=self.eliminar_categoria_gui, width=25).pack(pady=3)
        ttk.Button(frame, text="Cerrar", command=ventana.destroy, width=25).pack(pady=10)
    ...
    def registrar_categoria_gui(self):
        ventana = tk.Toplevel(self.root)
        ...
        ttk.Label(frame, text="Registrar Categoría", font=("Arial", 12, "bold")).pack(pady=5)

        ttk.Label(frame, text="Nombre de la categoría:").pack(anchor="w", pady=2)
        entry_nombre = ttk.Entry(frame, width=40)
        entry_nombre.pack(anchor="w", pady=2)

        def guardar():
            try:
                self.dao_categorias.registrar(entry_nombre.get())
                messagebox.showinfo("Éxito", "Categoría registrada con éxito")
                ventana.destroy()
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo registrar: {e}")
    ...
    def actualizar_categoria_gui(self):
        ventana = tk.Toplevel(self.root)
        ventana.title("Actualizar Categoría")
        ventana.geometry("400x220")
        ventana.resizable(False, False)
        ...
        ttk.Label(frame, text="Actualizar Categoría", font=("Arial", 12, "bold")).pack(pady=5)
        ...
        def guardar():
            try:
                nombre = entry_nombre.get().strip() or None
                self.dao_categorias.actualizar(int(entry_id.get()), nombre)
                messagebox.showinfo("Éxito", "Categoría actualizada con éxito")
                ventana.destroy()
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo actualizar: {e}")
    ...
    def eliminar_categoria_gui(self):
        ventana = tk.Toplevel(self.root)
        ...
        ttk.Label(frame, text="Eliminar Categoría", font=("Arial", 12, "bold")).pack(pady=5)
        ...
        def eliminar():
            try:
                self.dao_categorias.eliminar(int(entry_id.get()))
    ...
    def menu_ventas(self):
        ...
        ttk.Button(frame, text="Ver ventas con detalle de productos", command=self.ver_ventas_detalle_productos, width=25).pack(pady=3)
    ...
    def ver_ventas_detalle_productos(self):
        """Muestra ventas con ID de producto y cantidad comprada,
        para tener a mano los datos que pide 'Registrar devolución'."""
        rows = self.dao_ventas.consultar_detalle_productos()
        if not rows:
            self.mostrar_resultados("No hay ventas con detalle de productos.")
            return
        self.mostrar_tabla(
            ["ID Venta", "Fecha", "Cédula", "Cliente", "ID Producto", "Producto", "Cantidad"],
            rows, "--- Ventas con Detalle de Productos ---"
        )
    ...
    def menu_devoluciones(self):
        ...
        ttk.Button(frame, text="Registrar devolución", command=self.registrar_devolucion_gui, width=25).pack(pady=3)
        ttk.Button(frame, text="Ver devoluciones por cliente", command=self.ver_devoluciones_cliente_gui, width=25).pack(pady=3)
        ttk.Button(frame, text="Ver detalle de devoluciones (con IDs)", command=self.ver_detalle_completo_devoluciones_gui, width=25).pack(pady=3)
    ...
    def registrar_devolucion_gui(self):
        ...
        ttk.Label(frame, text="Registrar Devolución", font=("Arial", 12, "bold")).pack(pady=5)
        ...
        campos = [
            ...
            ("ID Tipo de Devolución:", "id_tipo_devolucion"),
            ("Motivo:", "motivo"),
        ]
        ...
        def guardar():
            try:
                self.dao_devoluciones.registrar(
                    int(entries["id_venta"].get()),
                    int(entries["id_producto"].get()),
                    entries["cedula"].get(),
                    int(entries["cantidad"].get()),
                    int(entries["id_tipo_devolucion"].get()),
                    entries["motivo"].get()
                )
    ...
    def ver_devoluciones_cliente_gui(self):
        ventana = tk.Toplevel(self.root)
        ventana.title("Devoluciones por Cliente")
        ventana.geometry("400x180")
        ventana.resizable(False, False)
        ...
        ttk.Label(frame, text="Devoluciones por Cliente", font=("Arial", 12, "bold")).pack(pady=5)
        ...
        def ver():
            try:
                rows = self.dao_devoluciones.consultar_por_cliente(entry_cedula.get())
                if not rows:
                    self.mostrar_resultados("Este cliente no tiene devoluciones registradas.")
                    return
                self.mostrar_tabla(
                    ["ID", "Motivo", "Cantidad", "Fecha", "Producto", "Venta", "Tipo"],
                    rows, "--- Devoluciones del Cliente ---"
                )
                ventana.destroy()
            except Exception as e:
    ...
    def ver_detalle_devoluciones_gui(self):
        rows = self.dao_devoluciones.ver_detalle_todas()
        if not rows:
            self.mostrar_resultados("No hay devoluciones registradas.")
            return
        self.mostrar_tabla(
            ["Fecha", "Cédula", "Cliente", "Producto", "Cantidad", "Motivo", "Tipo"],
            rows, "--- Detalle de Devoluciones ---"
        )

    def ver_detalle_completo_devoluciones_gui(self):
        """Muestra el detalle de devoluciones INCLUYENDO los IDs
        (ID_Devolucion, ID_Venta, ID_Producto, ID_Tipo_Devolucion),
        para saber qué valores usar en 'Registrar devolución'."""
        rows = self.dao_devoluciones.ver_detalle_completo()
        if not rows:
            self.mostrar_resultados("No hay devoluciones registradas.")
            return
        self.mostrar_tabla(
            ["ID Dev.", "Fecha", "Cédula", "Cliente", "ID Venta",
             "ID Prod.", "Producto", "Cantidad", "Motivo", "ID Tipo", "Tipo"],
            rows, "--- Detalle Completo de Devoluciones (con IDs) ---"
        )
